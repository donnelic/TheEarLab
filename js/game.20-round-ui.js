...
const updateModeVisibility = () => {
    ...
    const sheetVisible = state.active && getIsSheetRound();
    if (typingZone) {
        typingZone.hidden = !typingVisible;
        typingZone.style.display = typingVisible ? "" : "none";
    }
    if (sheetDisplay) {
        sheetDisplay.hidden = !sheetVisible;
        sheetDisplay.style.display = sheetVisible ? "" : "none";
    }
    if (statusPanel) {
    ...
    const measures = Array.from({ length: measureCount }, (_, measureIndex) => {
        ...
        return {
            ...
            notes: chunk.map((note, noteIndex) => {
                const centerX = measureIndex * measureWidth + (measureWidth * 0.5);
...
const renderSheetNotation = (notation) => {
    const clefSymbol = notation.clef === "bass" ? "𝄢" : "𝄞";
    const measureCount = notation.measures.length;
    const measuresHtml = notation.measures.map((measure, index) => `
    ...
    `).join("");
    const notesHtml = notation.measures.flatMap((measure) => measure.notes.map((note, noteIndex) => {
        ...
        const accidentalMarkup = accidentalGlyph
            ? `<span class="sheet-accidental" aria-hidden="true">${escapeHtml(accidentalGlyph)}</span>`
            : "";
        const label = note.label || note.noteId || "note";
        return `
            <div class="sheet-note" style="--sheet-note-left:${note.x}%; --sheet-note-top:${note.y}%; --sheet-note-z:${noteIndex};" data-note-id="${escapeHtml(note.noteId)}" aria-label="${escapeHtml(label)}">
                ${accidentalMarkup}
                <span class="sheet-note-head"></span>
                <span class="sheet-note-stem ${note.stemDirection}"></span>
                ${ledgerHtml}
            </div>
        `;
    })).join("");
    ...
    return `
        <div class="sheet-notation-stage" style="min-width:${notation.minWidth}px;">
            <div class="sheet-notation-grid" aria-hidden="true">
                <div class="sheet-staff-lines"></div>
                ${measuresHtml}
            </div>
            <div class="sheet-notation-meta">
                <div class="sheet-clef">${clefSymbol}</div>
                <div class="sheet-time-signature">
                    <span class="sheet-time-beats">${notation.timeSignature.beats}</span>
                    <span class="sheet-time-unit">${notation.timeSignature.unit}</span>
                </div>
            </div>
...
const renderSheetDisplay = () => {
    ...
    if (!state.active || !getIsSheetRound()) {
        sheetDisplay.innerHTML = "";
        return;
    }

    const clef = state.sheetClef === "bass" ? "bass" : "treble";
...
const updateStatus = () => {
    ...
    } else if (state.practiceMode === "sheet") {
        modeLabelEl.textContent = MODE_COPY.sheet || "Sheet Music";
    } else {
    ...
    if (!state.active) {
        ...
        resultEl.textContent = isTypingOnlyMode()
            ...
            : (state.practiceMode === "sheet"
                ? (PROMPT_COPY.landingSheet || "Press New Round to read the sheet, then select the notes on the keyboard.")
    ...
    if (!state.submitted && state.practiceMode === "sheet") {
        resultEl.textContent = PROMPT_COPY.sheetRound || "Read the sheet and select the notes on the keyboard. Hint plays the notes.";
    }
    if (hideLivePreview) {
