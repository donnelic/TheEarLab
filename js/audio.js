...
const ensureAudio = () => {
    ...
    if (!masterGain) {
        ...
        masterGain.gain.value = Math.pow(state.volume, 1.8) * 0.6;
...
const buildResonators = (ctx, preset, frequency, startTime, scaledVolume, ampNode) => {
    if (!Array.isArray(preset.resonators)) return { entries: [], stops: [] };
    const entries = [];
    const stops = [];
    preset.resonators.forEach((res) => {
        if (!Number.isFinite(res.ratio) || !Number.isFinite(res.gain)) return;
        const bp = ctx.createBiquadFilter();
        bp.type = "bandpass";
        bp.frequency.value = frequency * res.ratio;
        bp.Q.value = Number.isFinite(res.q) ? res.q : 12;
        const resGain = ctx.createGain();
        const level = res.gain * scaledVolume;
        resGain.gain.setValueAtTime(level, startTime);
        const tail = Math.max(0.15, res.decay ?? 1.0);
        resGain.gain.exponentialRampToValueAtTime(ENVELOPE_EPS, startTime + tail);
        bp.connect(resGain);
        resGain.connect(ampNode);
        entries.push({
            osc: bp,
            oscGain: resGain,
            adsrMults: { decay: 1 / (tail || 1), release: 1 },
            baseGain: level
        });
        stops.push({ node: resGain, stopTime: startTime + tail });
    });
...
const playPianoNote = (
    ...
    const stopTime = releaseEnd + 0.15;

    const oscillators = [];
    const oscStops = [];
    const oscGains = [];

    const maybeAttackNoise = () => {
        if (!preset.attackNoise?.level) return;
        applyNoiseBurst(
            ctx,
            amp,
            startTime,
            preset.attackNoise.level * scaledVolume,
            preset.attackNoise.decay ?? 0.02,
            {
                filterType: "highpass",
                centerFreq: preset.attackNoise.hp ?? 1500,
                q: 0.8
            }
        );
    };
    ...
    const addBody = () => {
        const filter = ctx.createBiquadFilter();
        filter.type = "lowpass";
        filter.frequency.value = preset.filterFrequency ?? 9000;
    ...
    const addPartials = () => {
        const parts = Array.isArray(preset.partials) ? preset.partials : [];
        if (!parts.length && Array.isArray(preset.oscillators)) {
        ...
        parts.forEach((oscSpec) => {
            for (let i = 0; i < unisonVoices; i += 1) {
                ...
                osc.frequency.value = frequency * (oscSpec.ratio ?? 1);
                const detune = Number.isFinite(detuneCents[i]) ? detuneCents[i] : 0;
                osc.detune.value = (oscSpec.detune ?? 0) + detune;
                const levelScale = scaledVolume * (oscSpec.gain ?? 1) * (1 / unisonVoices);
                ...
                oscGains.push({ gainNode: oscGain, adsrMults, baseGain: oscSpec.gain ?? 1 });
                osc.start(startTime);
                oscStops.push({ node: osc, stopTime });
            }
        });
    };

    const addResonators = () => {
        if (!Array.isArray(preset.resonators) || !preset.resonators.length) return;
        const burst = createNoise(ctx, 0.02);
        const burstGain = ctx.createGain();
        burstGain.gain.setValueAtTime(0.35 * scaledVolume, startTime);
        ...
        preset.resonators.forEach((res) => {
            ...
            bp.connect(resGain);
            resGain.connect(amp);
            oscStops.push({ node: resGain, stopTime: startTime + tail });
        });
        burst.start(startTime);
        burst.stop(startTime + 0.05);
        oscStops.push({ node: burst, stopTime: startTime + 0.05 });
    };
    ...
    const addFM = () => {
        ...
        carrier.type = getOscType(fm.carrierType ?? "sine");
        mod.type = getOscType(fm.modType ?? "sine");
        carrier.frequency.value = frequency;
        mod.frequency.value = frequency * (fm.modRatio ?? 2);
        modGain.gain.setValueAtTime((fm.modIndex ?? 3) * frequency, startTime);
        mod.connect(modGain);
        modGain.connect(carrier.frequency);
        const oscGain = ctx.createGain();
        scheduleEnvelope(oscGain.gain, startTime, duration, scaledVolume, false, {});
        carrier.connect(oscGain);
        if (preset.filter?.frequency) {
            const vcf = ctx.createBiquadFilter();
            vcf.type = "lowpass";
            vcf.frequency.value = preset.filter.frequency;
            vcf.Q.value = Number.isFinite(preset.filter.q) ? preset.filter.q : 0.7;
            oscGain.connect(vcf);
            vcf.connect(amp);
        } else {
            oscGain.connect(amp);
        ...
        oscGains.push({ gainNode: oscGain, adsrMults: {}, baseGain: 1 });
        if (preset.tremolo?.depth) {
            const trem = ctx.createOscillator();
            const tremGain = ctx.createGain();
            trem.type = "sine";
            trem.frequency.value = preset.tremolo.rate ?? 5;
            tremGain.gain.value = preset.tremolo.depth;
        ...
        mod.start(startTime);
        oscStops.push({ node: carrier, stopTime });
        oscStops.push({ node: mod, stopTime });
        if (preset.tremolo?.depth) {
            oscStops.push({ node: tremGain, stopTime });
            oscStops.push({ node: trem, stopTime });
        }
    };

    const engine = preset.engine ?? "modal";
    maybeAttackNoise();
    ...
    oscStops.forEach(({ node, stopTime: t }) => {
        setTimeout(() => {
            try {
                node.disconnect();
            } catch (e) {
    ...
    const voice = {
        gain: amp,
        oscs: oscillators.filter((o) => typeof o.stop === "function"),
        oscGains,
...
const playNotes = (noteIds, mode, startTime, options = {}) => {
    ...
    const volumeScale = 0.8 / Math.max(1, Math.sqrt(ids.length));
